...
let isMoving = false;
let moveProgress = 0;
const moveDuration = 2000; // 原有精灵移动时间延长
let moveStartTime = 0;
...
const movePath = [
  ...
  new THREE.Vector3(0.25, 4, 0),
  new THREE.Vector3(1, -1, 0),
  new THREE.Vector3(4, -1, 0.25),
...
// 神秘风格光源优化
const ambientLight = new THREE.AmbientLight(0x8888aa, 0.4); // 冷色调环境光
scene.add(ambientLight);

const pointLight = new THREE.PointLight(0x9999ff, 0.6, 20);
...
// 添加神秘氛围点光源
const mysteryLight = new THREE.PointLight(0xaa66ff, 0.2, 30);
mysteryLight.position.set(5, 15, -5);
mysteryLight.intensity = 0.3;
scene.add(mysteryLight);
...
function loadSpriteB() {
  loader.load(
    ...
    (gltf) => {
      spriteB = gltf.scene;
      spriteB.scale.set(0.6, 0.6, 0.6);
      spriteB.position.set(0, 3, 6);
      scene.add(spriteB);
...
function animate() {
  ...
  if (isPillarRising && targetPillar) {
    ...
    targetPillar.position.y = pillarInitY + easeProgress * 8; // 升高8单位
